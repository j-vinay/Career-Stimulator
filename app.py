...
# Add an alumni video
@app.route('/api/alumni-videos/add', methods=['POST'])
def add_alumni_video():
    data = request.json
    if not data.get('url') or not data.get('description'):
        return jsonify({'error': 'URL and description are required.'}), 400

    new_video = AlumniVideo(url=data['url'], description=data['description'])
    db.session.add(new_video)
    db.session.commit()

    return jsonify({'message': 'Alumni video added successfully!', 'id': new_video.id})
...
def get_alumni_videos():
    ...
    return jsonify([
        {'id': video.id, 'url': video.url, 'description': video.description, 'created_at': video.created_at}
        for video in videos
    ])

# Delete an alumni video by ID
